...
import { ID_API, WEATHER_API, TRANSLATE_API, GEOLOCATION_API } from './helpers/apiConstants';
import { BACKGROUND_API } from './helpers/apiConstants';
import { setNewBackImage, defineCurrentUnits } from './helpers/functionalConstants';
import { setActiveUnitsButtonFromStorage, setActiveLangFromStorage, currentDayState, currentWeatherPeriod } from './helpers/functionalConstants';
import { showError, hideError, handleLocationRequestError, handleImageRequestError, handleFetchResponse } from './helpers/functionalConstants';
	...
	function geoLocation(idCity) {
		...
		fetch(`${GEOLOCATION_API.base}json?q=${query || idCity}&key=${GEOLOCATION_API.key}&pretty=1&no_annotations=1`)
			...
			.then(res => {
				...
				translateApiData(res.results[0].components.county || res.results[0].components.state, setCityName);
